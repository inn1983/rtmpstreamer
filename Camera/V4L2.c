...
int openCameraDevice(void* v4l2ctx)
{
	...
	else if(tryFmt(v4l2ctx, V4L2_PIX_FMT_UYVY) == 0)
	{
		V4L2_Contect->mCaptureFormat = V4L2_PIX_FMT_UYVY;	// maybe easycap.
		LOGV("capture format: V4L2_PIX_FMT_UYVY");
	}
	else
	{
...
int v4l2SetVideoParams(void* v4l2ctx, int* width, int* height, int pix_fmt)
{
    ...
    if (V4L2_Contect->mCaptureFormat == V4L2_PIX_FMT_UYVY)
	{
